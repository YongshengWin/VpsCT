...
const userCols = `id, username, nickname, password_hash, role, enabled, avatar, totp_secret, totp_enabled, totp_last_step, recovery_codes, last_login_at, created_at, updated_at`
...
func scanUser(sc interface{ Scan(...any) error }) (domain.User, error) {
	...
	if err := sc.Scan(&u.ID, &u.Username, &u.Nickname, &u.PasswordHash, &u.Role, &enabled, &u.Avatar, &u.TOTPSecret, &totpEnabled, &u.TOTPLastStep, &recovery, &lastLogin, &created, &updated); err != nil {
		return u, err
	}
	u.Enabled = enabled == 1
...
func (s *Store) UpdateUser(ctx context.Context, u *domain.User) error {
	now := s.Now()
	_, err := s.db.ExecContext(ctx, `UPDATE users SET username=?, nickname=?, password_hash=?, role=?, enabled=?, avatar=?, totp_secret=?, totp_enabled=?, totp_last_step=?, recovery_codes=?, updated_at=? WHERE id=?`,
		u.Username, u.Nickname, u.PasswordHash, u.Role, b2i(u.Enabled), u.Avatar, u.TOTPSecret, b2i(u.TOTPEnabled), u.TOTPLastStep, recoveryJSON(u.RecoveryCodes), fmtTime(now), u.ID)
	u.UpdatedAt = now
	return err
}
...
func (s *Store) GetUserByName(ctx context.Context, name string) (domain.User, error) {
	u, err := scanUser(s.db.QueryRowContext(ctx, `SELECT `+userCols+` FROM users WHERE username=?`, name))
	if isNoRows(err) {
		return u, ErrNotFound
	}
	return u, err
...
func (s *Store) CreateSession(ctx context.Context, sess Session) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO sessions(id,user_id,created_at,expires_at,user_agent,ip) VALUES (?,?,?,?,?,?)`,
		sess.ID, sess.UserID, fmtTime(sess.CreatedAt), fmtTime(sess.ExpiresAt), sess.UserAgent, sess.IP)
	return err
}
...
func (s *Store) GetSession(ctx context.Context, id string) (Session, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT id,user_id,created_at,expires_at,user_agent,ip FROM sessions WHERE id=?`, id).
