...
```bash
make release VERSION=v0.1.1 REPOSITORY=YongshengWin/VpsCT
```
...
| `ctlvps-v0.1.1-linux-amd64.tar.gz`、`ctlvps-v0.1.1-linux-arm64.tar.gz` | 对应架构控制端、双架构 agent、服务配置、完整说明文档与许可文件 |
...
```bash
release_dir=release/v0.1.1 # 替换为刚用当前源码构建的待发布版本目录
...
```bash
sudo bash install.sh --assets-dir /path/to/release/v0.1.1 \
  --version v0.1.1 --domain panel.example.com
```
...
维护者确认后公开仓库和 Release。预发行标签可使用 `v0.1.1-rc.1`，发布时标记为 prerelease；正式用户的 `latest` 入口应指向正式版本。
