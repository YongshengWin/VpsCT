...
docker run -d --name "$name" --mount "type=volume,src=$volume,dst=/data" "$image" >/dev/null
ready=0
...
docker exec "$name" wget -qO- http://127.0.0.1:8080/api/v1/auth/setup | python3 -c 'import json,sys; assert json.load(sys.stdin)["needs_setup"]'
